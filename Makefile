...
MESH_SRC = src/mesh/atn_mesh.c
VOICE_SRC = src/voice/atn_voice.c src/voice/atn_voice_codec.c src/voice/atn_voice_jitter.c src/voice/atn_voice_e2e.c src/voice/atn_voice_dial.c
...
TEST_FUZZ = tests/test_fuzz$(EXE)
TEST_RECIPE = tests/test_recipe$(EXE)
TEST_HUB  = tests/test_hub_failover$(EXE)
TEST_VOICE = tests/test_voice$(EXE)
TEST_MESH = tests/test_mesh$(EXE)
CLI_2FA  = atn2fa$(EXE)
...
all: $(TEST_BIN) $(TEST_TUN) $(TEST_2FA) $(TEST_HTTP) $(TEST_DNS) $(TEST_TREE) $(TEST_REPL) $(TEST_HB) $(TEST_DMON) $(TEST_MLDSA) $(TEST_SIGN) $(TEST_CFG) $(TEST_FUZZ) $(TEST_RECIPE) $(TEST_HUB) $(TEST_VOICE) $(TEST_MESH) $(CLI_2FA) $(CLI_HTTP) $(CLI_DNS) $(CLI_SIGN) $(CLI_NODE) $(CLI_ENROLL)
...
$(TEST_HTTP): $(SRC) $(TUN_SRC) $(AUTH_SRC) $(HTTP_SRC) $(HB_SRC) $(SYNC_SRC) $(CFG_SRC) tests/test_http.c include/atn_http.h include/atn_cfg.h
	$(CC) $(CFLAGS) -o $@ $(SRC) $(TUN_SRC) $(AUTH_SRC) $(HTTP_SRC) $(HB_SRC) $(SYNC_SRC) $(CFG_SRC) tests/test_http.c $(LDFLAGS)
...
$(TEST_SIGN): $(SRC) $(SIGN_SRC) tests/test_sign.c include/atn_sign.h
	$(CC) $(CFLAGS) -o $@ $(SRC) $(SIGN_SRC) tests/test_sign.c $(LDFLAGS)
...
$(TEST_VOICE): $(SRC) $(TUN_SRC) $(VOICE_SRC) tests/test_voice.c include/atn_voice.h include/atn_tun.h
	$(CC) $(CFLAGS) -o $@ $(SRC) $(TUN_SRC) $(VOICE_SRC) tests/test_voice.c $(LDFLAGS) -lm

$(TEST_MESH): $(SRC) $(MESH_SRC) tests/test_mesh.c include/atn_mesh.h include/atn_tun.h
	$(CC) $(CFLAGS) -o $@ $(SRC) $(MESH_SRC) tests/test_mesh.c $(LDFLAGS)
...
test: $(TEST_BIN) $(TEST_TUN) $(TEST_2FA) $(TEST_HTTP) $(TEST_DNS) $(TEST_TREE) $(TEST_REPL) $(TEST_HB) $(TEST_DMON) $(TEST_MLDSA) $(TEST_SIGN) $(TEST_CFG) $(TEST_FUZZ) $(TEST_RECIPE) $(TEST_HUB) $(TEST_VOICE) $(TEST_MESH) $(CLI_2FA) $(CLI_HTTP) $(CLI_DNS) $(CLI_SIGN) $(CLI_NODE) $(CLI_ENROLL)
ifeq ($(CROSS),1)
...
clean:
	-rm -f $(TEST_BIN) $(TEST_TUN) $(TEST_2FA) $(TEST_HTTP) $(TEST_DNS) $(TEST_TREE) $(TEST_REPL) $(TEST_HB) $(TEST_DMON) $(TEST_MLDSA) $(TEST_SIGN) $(TEST_CFG) $(TEST_FUZZ) $(TEST_RECIPE) $(TEST_HUB) $(TEST_VOICE) $(TEST_MESH) $(CLI_2FA) $(CLI_HTTP) $(CLI_DNS) $(CLI_SIGN) $(CLI_NODE) $(CLI_ENROLL) $(LIB_BIN) atn_*.o android/libatn.so MANIFEST REPORT
	-cmd /c "del /Q tests\test_crypto.exe tests\test_tun.exe tests\test_2fa.exe tests\test_http.exe tests\test_dns.exe tests\test_tree.exe tests\test_repl.exe tests\test_hb.exe tests\test_dmon.exe tests\test_mldsa.exe tests\test_sign.exe tests\test_cfg.exe tests\test_fuzz.exe tests\test_recipe.exe tests\test_hub_failover.exe tests\test_voice.exe tests\test_mesh.exe atn2fa.exe atnhttp.exe atndns.exe atnsign.exe atnnode.exe atnenroll.exe libatn_crypto.a atn_*.o 2>NUL"
...
DAEMON_JAVA = \
	...
	android/java/com/athanor/daemon/AtnMesh.java \
